...
struct Nike: View {
    ...
    // Functions created to set variable clothes to the correct String to pull the correct items from scraping Lines 22 - 30 - AF
    func setBottoms() {
        clothes = "Pants"
    }
    func setHoodies() {
    ...
    func updateUserShirt() {
        ...
        let genderChart = (currentUserMeasurements?.gender == "Female" || currentUserMeasurements?.gender == "female") ? "womenShirtSizes" : "menShirtSizes"

        
        // 2. Fetching the chart from database
        
        let sizingChartRef = Firestore.firestore().collection("Nike_Chart").document(genderChart)
            ...
            if let currentUser = currentUserMeasurements {
                ...
                for (size, measurementsData) in data {
                    if let measurementsDict = measurementsData as? [String: Any] {
                        // Now, for each key-value in the dictionary, attempt to extract the array of Floats
                ...
                if let determinedSize = determinedSize {
                    ...
                    print("Could not determine user's shirt size.")
            ...
            if let currentUser = currentUserMeasurements {
                ...
                for (size, measurementsData) in data {
                    if let measurementsDict = measurementsData as? [String: Any] {
                        ...
                        //Chest will need to be commented out in the future.
                        if hipValue >= measurements["hip"]?[0] ?? 0 && hipValue <= measurements["hip"]?[1] ?? Float.infinity &&
                            waistValue >= measurements["waist"]?[0] ?? 0 && waistValue <= measurements["waist"]?[1] ?? Float.infinity {
                ...
                if let determinedSize = determinedSize {
                    ...
                    print("Could not determine user's pants size.")
                    print(currentUser.chest)
        ...
        NavigationView{
            VStack{
                ...
                Image("logo-nike")
                    ...
                    .frame(width: 100)
                    .offset(x:-130, y:70)
                // Dotted border was created to encompass the header banner image nike-banner that was custom created - AF
                ...
                VStack{
                    ...
                    List {
                        ...
                        NavigationLink(destination: NikeProducts( gender: $gender, clothes: $clothes, shirt: $shirt, pant: $pant).onAppear(perform: {
                            setHoodies()
                        }
                                                                                                         )){
                            Label {
